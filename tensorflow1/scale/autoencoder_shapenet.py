...
def random_transmats(batch_size):
    ...
    stl_transmat_inp = inp_3drotmat
    stl_transmat_trg = trg_3drotmat
...
def update_f_params(f_params, ax, theta):
    # do 3x3 rotation
    angles = np.zeros([1,3])
    angles[:,ax] = theta
    ...
    for step_i in xrange(num_steps):
        ...
        for i in xrange(max_angles):
            for j in xrange(max_angles):
                cur_f_params_j = update_f_params(cur_f_params, rot_ax, fangles[i*max_angles + j])
                feed_dict = {
                            test_x : cur_x,
...
def main(_):
    ...
    if opt['do_classify']:
        ...
        c_loss = 1000*tf.reduce_mean(c_loss)
