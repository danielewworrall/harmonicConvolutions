...
def zoom(N, image, factor):
	...
	new_coords = (int(image.shape[0]*0.5 - (N/2)), int(image.shape[1]*0.6 - (N/2)))
	return image[new_coords[0]:new_coords[0]+N, new_coords[1]:new_coords[1]+N]


def pyramid_conv(image, weight):
	"""Return an image pyramid"""
	from skimage.transform import pyramid_gaussian
	pyramid = tuple(pyramid_gaussian(image, downscale=1.2))
	mags = []
	phases = []
	for scale in pyramid:
		if scale.shape[0] > weight[0].shape[0]:
			y_r = conv(scale, weight[0], mode='valid')
			y_i = conv(scale, weight[1], mode='valid')
			y_r = cv2.resize(y_r, image.shape, interpolation=cv2.INTER_CUBIC)
			y_i = cv2.resize(y_i, image.shape, interpolation=cv2.INTER_CUBIC)
			
			mag = np.sqrt(y_r**2, y_i**2)
			phase = np.arctan2(y_i, y_r)
			mags.append(mag)
			phases.append(phase)
	mags = np.dstack(mags)
	phases = np.dstack(phases)
	return (mags, phases)
...
def main():
	...
	im_size = 200
	# Image
	im = skio.imread('../images/balloons.jpg')[0:300,100:400,:]
	im = skco.rgb2gray(im)
	...
	for i in xrange(30):		
		...
		image = zoom(im_size, im, scale_factor)
		mags, phases = pyramid_conv(image, weight)
		mags = np.mean(mags, axis=2)
		plt.imshow(mags)
		plt.draw()
		raw_input(i)
