...
def wide_resnet(opt, x, train_phase, device='/cpu:0'):
	...
	with tf.name_scope('gap') as scope:
		gap = tf.reduce_mean(res4, axis=[1,2])
...
def h_VGG(opt, x, train_phase, device='/cpu:0'):
	...
	with tf.name_scope('gap') as scope:
		...
		gap = tf.reduce_mean(mags, axis=[1,2])
		gapsh = gap.get_shape().as_list()
		gap = tf.reshape(gap, tf.stack([gapsh[0],-1]))
...
def mixed_VGG(opt, x, train_phase, device='/cpu:0'):
	...
	for i in xrange(5):
		...
		if i == opt['block_multiplicity']:
			...
			xsh = x.get_shape().as_list()
			x = tf.reshape(x, tf.stack([xsh[0],xsh[1],xsh[2],-1]))
		# Apply an h/Z=block
		if i < opt['block_multiplicity']:
	...
	xsh = x.get_shape().as_list()
	x = tf.reshape(x, tf.stack([xsh[0],-1]))
...
def range_VGG(opt, x, train_phase, device='/cpu:0'):
	...
	# Block 1
	res1_1 = hn_lite.range_conv2d(x, nf1, fs, in_range=(0,0), out_range=(0,1),
											n_rings=nr, padding='SAME', name='1_1', device=d)
	res1_1 = hn_lite.batch_norm(res1_1, tp, tf.nn.relu, name='n1_1', device=d)
	res1_2 = hn_lite.range_conv2d(res1_1, nf1, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='1_2', device=d)
	res1_2 = hn_lite.batch_norm(res1_2, tp, fnc=tf.nn.relu, name='n1_2', device=d)
	res1_mp = hn_lite.mean_pool(res1_2, ksize=(1,2,2,1), strides=(1,2,2,1), name='1_mp')
	
	# Block 2
	res2_1 = hn_lite.range_conv2d(res1_mp, nf2, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='2_1', device=d)
	res2_1 = hn_lite.batch_norm(res2_1, tp, tf.nn.relu, name='n2_1', device=d)
	res2_2 = hn_lite.range_conv2d(res2_1, nf2, fs, in_range=(0,1), out_range=(0,1),
	...
	# Block 3
	res3_1 = hn_lite.range_conv2d(res2_mp, nf3, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='3_1', device=d)
	res3_1 = hn_lite.batch_norm(res3_1, tp, tf.nn.relu, name='n3_1', device=d)
	res3_2 = hn_lite.range_conv2d(res3_1, nf3, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='3_2', device=d)
	res3_2 = hn_lite.batch_norm(res3_2, tp, fnc=tf.nn.relu, name='n3_2', device=d)
	
	# Block 4
	res4_1 = hn_lite.range_conv2d(res3_2, nf4, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='4_1', device=d)
	res4_1 = hn_lite.batch_norm(res4_1, tp, tf.nn.relu, name='n4_1', device=d)
	res4_2 = hn_lite.range_conv2d(res4_1, nf4, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='4_2', device=d)
	res4_2 = hn_lite.batch_norm(res4_2, tp, fnc=tf.nn.relu, name='n4_2', device=d)
	
	# Block 5
	res5_1 = hn_lite.range_conv2d(res4_2, nf5, fs, in_range=(0,1), out_range=(0,1),
											n_rings=nr, padding='SAME', name='5_1', device=d)
	res5_1 = hn_lite.batch_norm(res5_1, tp, tf.nn.relu, name='n5_1', device=d)
	res5_2 = hn_lite.range_conv2d(res5_1, nf5, fs, in_range=(0,1), out_range=(0,1),
	...
	res_mag = tf.reshape(res_mag, tf.stack([rsh[0],rsh[1],rsh[2],-1]))
	
	with tf.name_scope('gap') as scope:
		gap = tf.nn.relu(tf.reduce_mean(res_mag, axis=[1,2]))
		return tf.nn.bias_add(tf.matmul(gap, Wg), bg)
...
def mixed_VGG_(opt, x, train_phase, device='/cpu:0'):
	...
	res1_mag = tf.reshape(res1_mag, tf.stack([rsh[0],rsh[1],rsh[2],-1]))
	...
	with tf.name_scope('gap') as scope:
		gap = tf.nn.relu(tf.reduce_mean(res5_3, axis=[1,2]))
		return tf.nn.bias_add(tf.matmul(gap, Wg), bg)
