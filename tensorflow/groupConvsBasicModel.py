...
#define optimiser
graph_learning_rate = tf.placeholder(tf.float32, shape=[], name="learningRate")
step = tf.Variable(0, name='global_step', trainable=False)
optim = tf.train.AdamOptimizer(graph_learning_rate)
train_op = optim.minimize(loss, global_step=step)
learningRate_summary = tf.scalar_summary("LearningRate", graph_learning_rate)
